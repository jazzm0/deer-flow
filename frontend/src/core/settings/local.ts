...
export function getThreadMode(
    threadId: string,
): LocalSettings["context"]["mode"] {
    if (!isBrowser()) {
        return undefined;
    }
    const val = localStorage.getItem(getThreadModeStorageKey(threadId));
    if (
        val === "flash" ||
        val === "thinking" ||
        val === "pro" ||
...
export function applyThreadOverrides(
    ...
    threadMode: LocalSettings["context"]["mode"],
): LocalSettings {
    if (!threadModelName && !threadMode) {
        return settings;
    }
    return {
        ...settings,
